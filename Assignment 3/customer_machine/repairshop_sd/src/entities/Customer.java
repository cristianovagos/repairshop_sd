...
public class Customer extends Thread {
     ...
     * @param lounge referência para o Lounge {@link ILounge}
     * @param park referência para o Park {@link IPark}
     * @param outsideWorld referência para o Outside World {@link IOutsideWorld}
     */
    public Customer(int id, IGeneralRepository repo, ILounge lounge, IPark park, IOutsideWorld outsideWorld) throws RemoteException {
        this.customerId = id;
        this.carId = id;
    ...
    public void run() {
        try {
            decideOnRepair();
            park.goToRepairShop(customerId, carId);
            setState(CustomerState.PARK);
            lounge.queueIn(false, customerId);
            setState(CustomerState.RECEPTION);
            lounge.talkWithManager(customerId);
            setCarId(-1);
            setState(CustomerState.RECEPTION_TALK_WITH_MANAGER);
            if(wantsReplacementCar) {
                setState(CustomerState.WAITING_FOR_REPLACE_CAR);
                carId = lounge.collectKey(customerId);
                park.findCar(carId, customerId);
                setState(CustomerState.PARK);
                setState(CustomerState.NORMAL_LIFE_WITH_CAR);
                outsideWorld.backToWorkByCar(false, customerId);
                park.goToRepairShop(customerId, carId);
                setState(CustomerState.PARK);
            } else {
                setState(CustomerState.NORMAL_LIFE_WITHOUT_CAR);
                outsideWorld.backToWorkByBus(customerId);
            }
            lounge.queueIn(true, customerId);
            setState(CustomerState.RECEPTION_PAYING);
            lounge.payForTheService(customerId);
            park.collectCar(customerId);
            setState(CustomerState.PARK);
            setCarId(customerId);
            outsideWorld.backToWorkByCar(true, customerId);
        } catch (RemoteException e) {
            GenericIO.writelnString("Remote exception thrown on Customer lifecycle: " + e.getMessage());
            e.printStackTrace();
            System.exit(1);
        }
    }
